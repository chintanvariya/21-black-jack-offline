...
namespace I2.MiniGames
{
    ...
    public class MiniGame_Reward : MonoBehaviour
    {
        ...
        public UnityEngine.UI.Text coinValue;
        ...
        public virtual void Execute(MiniGame game, Transform parent)
        {
            ...
            Debug.LogError($" rewardedValue => {rewardedValue}RewardName => {this.name} ");
            CallBreakSpinnerController.GetReward(rewardedValue);
            //Debug.LogError($" rewardedValue => {rewardedValue }RewardName => {this.name} || {this.transform.GetChild(1).GetChild(0).GetComponent<UnityEngine.UI.Text>().text} ");
        }
        ...
        public virtual void Show(Transform parent)
        {
            ...
            coinValue.text = $"{CallBreakUtilities.AbbreviateNumber(rewardedValue)}";
